<script>
	...
	const p = x => {
		...
		return yootils.clamp((x - left) / (right - left), 0, 1);
	};
	...
	const reset = handler((start, p) => {
		min = yootils.clamp(Math.min(start.p, p), 0, 1);
		max = yootils.clamp(Math.max(start.p, p), 0, 1);
	});
	
	const move = handler((start, p) => {
		const d = yootils.clamp(p - start.p, -start.min, 1 - start.max);
		min = start.min + d;
		max = start.max + d;
	});
